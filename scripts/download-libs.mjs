...
function parseEsmShimRedirects(content, originUrl) {
  ...
  const re =
    /(?:export\s+\*\s+from|import|export\s+\{[^}]*\}\s+from)\s+["'](\/[^"']+)["']/g
  let match
...
async function downloadResolved(url) {
  ...
  if (data.length < 500 && fetchUrl.includes('esm.sh')) {
    ...
    if (redirectUrls.length > 0) {
      console.log(
        `    (shim detected, following ${redirectUrls.length} unique redirect(s))`,
      )
      const parts = []
...
async function downloadFonts(fonts) {
  ...
  for (const font of fonts) {
    ...
    let cssText
    try {
      const cssBuf = await download(cssUrl, 10, {
