...
3. **Reproduce with curl** – fire the relevant HTTP request(s) using Bash + curl against `http://localhost:5000` (or the port in `config.json`). Use `-s -w "\n%{http_code}"` to capture status and body together.
...
6. **Propose a fix** – describe the change needed. If the fix is straightforward, implement it, run `npx prettier . --write`, then re-run the curl to confirm.

## Constraints

- Always start the server (`npm start`) before firing requests if it is not already running. Check with `curl -s http://localhost:5000/status` first.
