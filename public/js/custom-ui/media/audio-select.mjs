...
export class AudioSelect extends Component {
  ...
  componentDidMount() {
    ...
    this.unsubscribePlayer = globalAudioPlayer.subscribe(() => {
      if (this.state.audioUrl) {
        this.setState({
          isPlaying: globalAudioPlayer.isPlaying(this.state.audioUrl),
        })
      }
    })
  ...
  componentWillUnmount() {
    ...
    if (
      this.state.audioUrl &&
      globalAudioPlayer.isPlaying(this.state.audioUrl)
    ) {
  ...
  render() {
    ...
    const { theme, audioUrl, audioName, albumImageUrl, isPlaying, isHovered } =
      this.state
