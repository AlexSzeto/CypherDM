...
export function ProgressBanner({
  ...
  const suppressTitleRef = useRef(suppressTitle)
  onCompleteRef.current = onComplete
  onErrorRef.current = onError
  onCancelledRef.current = onCancelled
  onProgressRef.current = onProgress
  onDismissRef.current = onDismiss
  suppressTitleRef.current = suppressTitle
  ...
  useEffect(() => {
    ...
    const handleProgressUpdate = (data) => {
      ...
      if (!suppressTitleRef.current) pageTitleManager.update(titleMessage)
      ...
      onProgressRef.current?.(data.progress.percentage)
    }
    ...
    const handleComplete = (data) => {
      log(
        'progress-banner',
        'info',
        `handleComplete fired for taskId=${taskId}`,
      )
      if (!suppressTitleRef.current) pageTitleManager.reset()
      ...
      if (onCompleteRef.current) {
        log(
          'progress-banner',
          'info',
          `calling onComplete prop for taskId=${taskId}`,
        )
        ...
        log(
          'progress-banner',
          'warn',
          `onComplete prop is null/undefined for taskId=${taskId}`,
        )
      }
    ...
    log('progress-banner', 'info', `subscribing to taskId=${taskId}`)
    ...
    log(
      'progress-banner',
      'info',
      `subscribe returned ${subscribed} for taskId=${taskId}`,
    )
    ...
    return () => {
      log(
        'progress-banner',
        'info',
        `useEffect cleanup: unsubscribing taskId=${taskId}`,
      )
  ...
  const handleCancelClick = useCallback(async () => {
    ...
    try {
      ...
      log('progress-banner', 'error', `Cancel failed: ${err}`)
    }
