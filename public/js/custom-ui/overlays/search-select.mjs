 ...
 *   mode="single"
 *   itemActions=${[
 *     { icon: 'copy', title: 'Clone', onClick: (item) => clone(item.value), closeAfter: true },
 *   ]}
 *   onSelect=${(value) => console.log('Selected:', value)}
 *   onClose=${() => setOpen(false)}
 * />
 ...
 * @param {Array<{icon:string, title?:string, color?:string, onClick:Function, closeAfter?:boolean, disabled?:boolean|Function}>} [props.itemActions] – Per-item action buttons rendered on the right of each row; `onClick` receives the normalised item ({ label, value, subtitle }); `closeAfter: true` closes the modal after the action runs
...
export function SearchSelectModal({
  ...
  const inputIdRef = useRef(
    'search-select-input-' + Math.random().toString(36).slice(2),
  )
  useEffect(() => {
    if (!isOpen) return
  ...
  const filtered = normalisedItems
    .filter(
      (item) =>
        ...
        item.subtitle.toLowerCase().includes(lowerFilter),
    )
  ...
  // stopPropagation prevents the row's select/toggle from firing
            ...
            `
                : filtered.map((item) => {
                  ...
                  >
                    ...
                    ${hasActions ? renderItemActions(item) : null}
                  </${MultiItem}>
                `
                    }
                    // Single-select mode with item actions: SingleItem is a
                    // <button>, and nesting the action <button>s inside it is
                    // invalid HTML — reuse the div-based MultiItem row instead
                    // (same hover/click behaviour, no checkbox).
                    if (hasActions) {
                      return html`
                  <${MultiItem}
                    key=${String(item.value)}
