...
export class NavigatorComponent extends Component {
  constructor(props) {
    ...
    if (
      typeof (props.itemsPerPage || 1) !== 'number' ||
      (props.itemsPerPage || 1) < 1
    ) {
    ...
    this.state = {
      ...
      totalPages: Math.max(
        1,
        Math.ceil((props.dataList || []).length / (props.itemsPerPage || 1)),
      ),
      theme: currentTheme.value,
    }
    ...
    log(
      'navigator',
      'info',
      `NavigatorComponent initialized: ${{
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (prevProps.dataList !== this.props.dataList) {
      ...
      const totalPages = Math.max(
        1,
        Math.ceil(newDataList.length / this.state.itemsPerPage),
      )
      ...
      log(
        'navigator',
        'info',
        `NavigatorComponent props.dataList changed, synced to state: ${{
    ...
    this.setState(
      ...
      () => {
        log(
          'navigator',
          'info',
          `NavigatorComponent data updated: ${{
            dataLength: newDataList.length,
            totalPages,
            currentPage,
          }}`,
  ...
  setItemsPerPage(newItemsPerPage) {
    ...
    const totalPages = Math.max(
      1,
      Math.ceil(this.state.dataList.length / newItemsPerPage),
    )
    const currentPage = Math.min(
      this.state.currentPage,
      Math.max(0, totalPages - 1),
  ...
  goToPage(pageIndex) {
    if (typeof pageIndex !== 'number') {
      throw new Error(
        `pageIndex must be a number, received: ${typeof pageIndex}`,
      )
    }
    ...
    if (pageIndex >= this.state.totalPages) {
      throw new Error(
        `pageIndex must be < ${this.state.totalPages} (total pages), received: ${pageIndex}. Current data length: ${this.state.dataList.length}`,
      )
    }
...
export function createNavigator(
  ...
  return {
    ...
    destroy() {
      ...
      navigatorRef = null
      log('navigator', 'info', 'NavigatorComponent destroyed')
    },
  }
}
