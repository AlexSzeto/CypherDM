 ...
 * @param {'normal'|'compact'|null} [props.heightScale=null] - Height override: null preserves the variant's own height; 'normal'/'compact' match the same heightScale on Input/Select (44px | 34px)
...
export class Button extends Component {
  ...
  render() {
    ...
    const sizes = {
      ...
      medium: {
        ...
        borderRadius: isIconOnly
          ? theme.spacing.small.borderRadius
          : theme.spacing.medium.borderRadius,
        gap: '8px',
      },
      large: {
        height: '44px',
    ...
    const { width: scaleWidth, flex: scaleFlex } = getWidthScaleStyle(
      widthScale,
      size.height,
    )
    const buttonWidth =
      widthScale !== 'auto' ? scaleWidth : isIconOnly ? size.height : undefined
    const buttonFlex =
      widthScale !== 'auto' ? scaleFlex || undefined : undefined
