...
const RangeInput = styled('input')`
  ...
  outline: none;
  transition:
    border-color ${(props) => props.transition},
    box-shadow ${(props) => props.transition};
...
export class RangeSlider extends Component {
  constructor(props) {
    super(props)
    const { minAllowed = 0, maxAllowed = 100, min, max } = props
    this.state = {
  ...
  componentDidUpdate(prevProps) {
    const { minAllowed = 0, maxAllowed = 100, min, max } = this.props

    // Sync controlled min/max props when they change externally (e.g. revert).
    if (min !== undefined && min !== prevProps.min) {
      this.setState({
        currentMin: Math.max(minAllowed, Math.min(min, maxAllowed)),
      })
    }
    if (max !== undefined && max !== prevProps.max) {
      this.setState({
        currentMax: Math.max(minAllowed, Math.min(max, maxAllowed)),
      })
    }

    // Re-clamp both handles when the allowed range itself changes.
    if (
      prevProps.minAllowed === minAllowed &&
      prevProps.maxAllowed === maxAllowed
    )
      return
  ...
  handleMinChange(e) {
    const { maxAllowed = 100, onChange } = this.props
  ...
  handleMaxChange(e) {
    const { minAllowed = 0, onChange } = this.props
    const raw = parseFloat(e.target.value)
